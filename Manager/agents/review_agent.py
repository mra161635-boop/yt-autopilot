...
from config import ANTHROPIC_API_KEY, CHANNEL_NICHE, CHANNEL_NAME, CLAUDE_MODEL
...
def analyse_and_update_strategy(videos: list[dict], comments: dict[str, list[str]]):
    """Send all data to Claude; get back an updated strategy memo."""
...
"""
    ...
    new_memo = resp.content[0].text.strip()
...
def channel_health_check(videos: list[dict]) -> list[str]:
    ...
    alerts = []
    if not videos:
        alerts.append("⚠️  No videos published yet.")
        return alerts
    ...
    if missing_thumb:
        alerts.append(f"🖼️  Videos missing custom thumbnails: {missing_thumb}")

    if len(videos) < 5:
        alerts.append("📅 Channel is young. Keep publishing consistently.")

    if not alerts:
        alerts.append("✅ Channel health looks good.")
